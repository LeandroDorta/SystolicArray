...
generate
	for (row=0; row<4; row=row+1) begin
		assign matrix_row_msg[row] = input_matrix_tdata[row*C_DATA_BIT_WIDTH+:C_DATA_BIT_WIDTH];
	end
  for (row=0; row<4; row=row+1) begin
    assign matrix_row_en[row] = input_matrix_tvalid;
  end
	for (col=0; col<4; col=col+1) begin
		assign weight_in_col_msg[col] = weight_matrix_tdata[col*C_DATA_BIT_WIDTH+:C_DATA_BIT_WIDTH];
	end
  for (col=0; col<4; col=col+1) begin
    assign weight_in_col_en[col] = weight_matrix_tvalid;
  end
	for (col=0; col<4; col=col+1) begin
		assign output_matrix_tdata[col*C_DATA_BIT_WIDTH+:C_DATA_BIT_WIDTH] = matrix_col_msg[col];
	end
	for (col=0; col<4; col=col+1) begin
...
assign input_matrix_tready = matrix_row_rdy[0] && matrix_row_rdy[1] && matrix_row_rdy[2] && matrix_row_rdy[3] && all_weights_avail;
assign weight_matrix_tready = weight_in_col_rdy[0] && weight_in_col_rdy[1] && weight_in_col_rdy[2] && weight_in_col_rdy[3] && !all_weights_avail;
assign output_matrix_tvalid = matrix_col_en[0] && matrix_col_en[1] && matrix_col_en[2] && matrix_col_en[3];
assign weight_out_col_rdy[0] = 1;
assign weight_out_col_rdy[1] = 1;
assign weight_out_col_rdy[2] = 1;
assign weight_out_col_rdy[3] = 1;
